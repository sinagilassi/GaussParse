...
# local
from gpkit import create_excel_file
...
if uploaded_files is not None:
    ...
    # add button
    download_btn = st.sidebar.button("Download Excel File")

    if download_btn:
        # read uploaded_files
        with io.BytesIO(uploaded_files.getvalue()) as f:
            # read uploaded_files as a txt
            data = f.read().decode('utf-8')

        res, dfs = gp.result_summary_to_excel(data)
        # display res
        st.write(res)
        # display dataframe
        st.write(dfs)
